...
HairOnly<- select(Students,Hair,-Sex)
...
StudentsNew<-data.frame("Hair"=Students$Hair,
                        "Eye"=Students$Eye,
                        "Gender"=Students$Sex,
                        "Freq"=Students$Freq)
...
MaleOnly<-filter(StudentsNew,Gender=="Male")


#Let's 'group' our data by gender
#TASK: Write the function to group the data by gender (hint: group_by)
group_by(StudentsNew,Gender)

#Let's see how many students were examined in the dataset (total the frequency)
#TASK: replace 'datasetname' with the name of yoursee dataset and get the total
#After you run it, write the total here:592
total=mutate(StudentsNew, total=cumsum(Freq))


#Since we have a males dataset, let's make a females dataset
#TASK: Write the function that includes only rows that are 'female'
FemaleOnly<-filter(StudentsNew,Gender=="Female")
